...
# - Auto GW detection (uses NEXT GW after the current GW's deadline has passed)
...
# - Shows ADOPTED moves (and candidate diagnostics) to avoid confusion
...
SQUAD15_SIZE = 15
START11_SIZE = 11
BENCH_WEIGHT = 0.35          # bench points worth 35% of starter points
NET_GAIN_THRESHOLD = 2.0     # adopt transfers only if net XI gain ≥ 2.0 points
...
def fetch_effective_gw():
    """
    Returns the effective GW id for optimisation:
    - If the API says 'is_current' and deadline not passed, use it.
    - If the current GW deadline has already passed, auto-switch to 'is_next'
      so you're always planning for the upcoming GW.
    - Falls back to 'next' or last GW if needed.
    """
    ...
    if cur:
        ...
        # If we're past the current GW's deadline, plan for NEXT
    ...
    if nxt:
        return nxt["id"]
    return events[-1]["id"]  # last resort
...
class FPLApp(tk.Tk):
    ...
    def build_ui(self):
        ...
        self.trans_text = tk.Text(tf, height=10, width=140)
    ...
    def run(self):
        ...
        if not curr15:
            ...
            # Candidate via proactive optimiser (hits always apply outside the 1 FT)
        ...
        if curr15:
            ...
            net_now        = after_pts_now - before_pts_now - penalty

            cand_after     = xi_points(candidate15_df)[0] if curr15 else 0.0
            cand_net       = cand_after - before_pts - candidate_penalty if curr15 else 0.0

            self.trans_text.insert(tk.END,
                "ADOPTED (after hits & ≥ threshold):\n"
                ...
                f"  Threshold: {NET_GAIN_THRESHOLD:.1f}\n"
                "\nCANDIDATE (best before threshold check):\n"
